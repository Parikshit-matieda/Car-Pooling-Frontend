...
const UserDashboard: React.FC = () => {
    ...
    const stats = [
        { label: 'Rides Offered', value: '12', icon: '🚗', color: 'bg-yellow-100 text-black' },
        { label: 'Rides Taken', value: '4', icon: '🎫', color: 'bg-yellow-100 text-black' },
        { label: 'Carbon Saved', value: '52kg', icon: '🌿', color: 'bg-green-100 text-green-700' },
    ];
            ...
            <div className="max-w-7xl mx-auto px-4 py-12 md:py-20">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                    ...
                                    <div key={i} className="flex gap-8 items-start pb-10 border-b border-gray-50 last:border-0 last:pb-0 group">
                                        ...
                                        <div>
                                            <p className="text-black font-black text-lg leading-tight mb-2">
                                                {i === 1 ? 'You offered a ride to Mumbai Central' : 'You received a 5-star rating from Rahul'}
                                            </p>
                                            <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest italic">Yesterday at 4:30 PM</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-white rounded-[40px] border border-black/5 p-10 shadow-sm animate-fade-in">
                            <h2 className="text-2xl font-black text-black tracking-tight mb-8">Up Next</h2>
                            {upcomingRides.length === 0 ? (
                                <div className="text-center py-12">
                                    <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center text-4xl mx-auto mb-6 opacity-30">
                                        📅
                                    </div>
                                    <p className="text-black/40 font-black text-sm uppercase tracking-widest">No rides booked</p>
                                    <button
                                        onClick={() => navigate('/search')}
                                        className="bg-gray-50 text-black px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest mt-8 hover:bg-yellow-100 transition-colors w-full"
                                    >
                                        Find a ride ⚡
                                    </button>
                                </div>
