    ...
    const RideCard = ({ ride, isExpanded, onToggleExpand }: { ride: any; isExpanded: boolean; onToggleExpand: () => void }) => {
        ...
        return (
            <div className={`bg-white rounded-[32px] sm:rounded-[40px] p-5 sm:p-8 border border-black/5 shadow-sm hover:shadow-xl transition-all relative overflow-hidden ${isCancelled ? 'opacity-75 grayscale-[0.5]' : ''}`}>
                {isCancelled && <div className="absolute top-0 left-0 bg-red-500 text-white px-6 py-2 rounded-br-[24px] font-black text-[10px] uppercase tracking-widest z-10">Cancelled</div>}
                {isCompleted && <div className="absolute top-0 left-0 bg-green-500 text-white px-6 py-2 rounded-br-[24px] font-black text-[10px] uppercase tracking-widest z-10">Completed</div>}
                {!isCancelled && !isCompleted && (
                    <div className={`absolute top-0 left-0 ${labelColors[dateLabel as keyof typeof labelColors]} px-6 py-2 rounded-br-[24px] font-black text-[10px] uppercase tracking-widest z-10`}>{dateLabel}</div>
                )}
                ...
                <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-16 gap-8 text-center md:text-left">
                    <div className="space-y-6 flex flex-col items-center md:items-start">
                        <div>
                            ...
                            <h1 className="text-4xl sm:text-6xl font-[1000] text-black tracking-tighter leading-none text-center md:text-left">
                    ...
                    <div className="w-full flex justify-center md:justify-end overflow-x-auto pb-4 md:pb-0 no-scrollbar">
