...
const RideCreationFlow: React.FC = () => {
    ...
    const [routes, setRoutes] = useState<{ coords: [number, number][], distance: number, duration: number }[]>([]);
    const [selectedRouteIdx, setSelectedRouteIdx] = useState(0);
    const [finalLegPrice, setFinalLegPrice] = useState('0');
    const [fullJourneyPrice, setFullJourneyPrice] = useState('0');
    ...
    if (user?.license_status !== 'VERIFIED') return (
        <div className="min-h-screen bg-gray-50 flex flex-col uppercase">
            <Navbar />
            <div className="flex-1 flex items-center justify-center p-4">
                <div className="bg-white p-12 rounded-3xl shadow-xl text-center border border-red-50 max-w-md w-full">
                    <div className="text-5xl mb-6">🪪</div>
                    <h2 className="text-2xl font-black mb-4">Verification Required</h2>
                    <p className="text-gray-500 font-bold mb-8">Your driving license must be verified before you can offer rides.</p>
                    <button onClick={() => navigate('/upload-license')} className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-lg">Upload License</button>
                </div>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col uppercase font-sans">
            ...
            <div className="bg-[#f7d302] border-b border-black/5 px-4 py-8 sticky top-20 z-40 shadow-sm">
                ...
                <div className="max-w-4xl mx-auto mt-6 flex justify-between px-1">
                    <span className={`text-[10px] font-black uppercase tracking-widest ${step >= 1 ? 'text-black' : 'text-black/30'}`}>Pickup Search</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${step >= 2 ? 'text-black' : 'text-black/30'}`}>Map Pivot</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${step >= 3 ? 'text-black' : 'text-black/30'}`}>Route Choice</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${step >= 4 ? 'text-black' : 'text-black/30'}`}>Stopovers</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${step >= 5 ? 'text-black' : 'text-black/30'}`}>Schedule</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${step >= 6 ? 'text-black' : 'text-black/30'}`}>Pricing</span>
            ...
            <div className="flex-1 max-w-5xl mx-auto w-full p-4 md:p-8">
                ...
                {step === 1 && (
                    <div className="bg-white rounded-[48px] shadow-2xl p-10 md:p-16 border border-black/5 animate-fade-in">
                        ...
                        <div className="space-y-10">
                            <div className="relative">
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-2">Starting Point</label>
                            ...
                            <div className="relative">
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-2">Destination</label>
                                <div className="relative group">
                                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-3xl group-focus-within:scale-110 transition-all">🎯</span>
                                    <input
                                        type="text"
                ...
                {step === 2 && (
                    <div className="bg-white rounded-[48px] shadow-2xl overflow-hidden border border-black/5 flex flex-col min-h-[70vh] animate-fade-in relative">
                        <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-black text-white">
                            <div>
                                <h3 className="text-2xl font-black tracking-tight">Finalize Points</h3>
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={fetchRoute} disabled={loading} className="px-10 py-3 bg-[#f7d302] text-black font-black rounded-2xl hover:scale-105 transition-all text-xs uppercase tracking-widest shadow-xl shadow-[#f7d302]/20 flex items-center gap-2">
                                    {loading ? <div className="w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin"></div> : 'Discovery Route ➔'}
                                </button>
                            </div>
                        </div>
                                    ...
                                    <Marker
                                        ...
                                        eventHandlers={{
                                            dragend: (e) => {
                                                ...
                                                setSource({ ...source, lat: position.lat, lng: position.lng });
                                            },
                                        }}
                                    />
                                )}
                                    ...
                                    <Marker
                                        ...
                                        eventHandlers={{
                                            dragend: (e) => {
                                                const marker = e.target;
                                                const position = marker.getLatLng();
                                                setDestination({ ...destination, lat: position.lat, lng: position.lng });
                                            },
                                        }}
                                    />
                                )}
                ...
                {step === 3 && (
                    <div className="bg-white rounded-[48px] shadow-2xl overflow-hidden border border-black/5 flex flex-col min-h-[70vh] animate-fade-in lg:flex-row">
                        <div className="lg:w-96 bg-gray-50 p-8 flex flex-col border-r border-gray-100">
                                ...
                                {routes.map((r, idx) => (
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-4">
                                            <span className={`text-[10px] font-black uppercase tracking-widest ${selectedRouteIdx === idx ? 'text-[#f7d302]' : 'text-gray-400'}`}>Route Choice {idx + 1}</span>
                            ...
                            <div className="mt-8 space-y-3">
                                <button onClick={nextStep} className="w-full py-5 bg-[#f7d302] text-black font-black rounded-2xl shadow-xl shadow-[#f7d302]/20 hover:scale-[1.02] transition-all uppercase text-sm tracking-widest" disabled={routes.length === 0}>Confirm Pathway ➔</button>
                ...
                {step === 4 && (
                    <div className="bg-white rounded-[48px] shadow-2xl overflow-hidden border border-black/5 flex flex-col min-h-[70vh] animate-fade-in lg:flex-row">
                        <div className="lg:w-96 bg-gray-50 p-8 flex flex-col border-r border-gray-100">
                            <h3 className="text-2xl font-black text-black mb-2 tracking-tight">Stopovers</h3>
                            ...
                            <div className="mt-8 space-y-3">
                                <button onClick={nextStep} className="w-full py-5 bg-[#f7d302] text-black font-black rounded-2xl shadow-xl shadow-[#f7d302]/20 hover:scale-[1.02] transition-all uppercase text-sm tracking-widest">Next: Timing ➔</button>
                ...
                {step === 5 && (
                    <div className="bg-white rounded-[48px] shadow-2xl p-10 md:p-16 border border-black/5 animate-fade-in max-w-3xl mx-auto">
                        <h2 className="text-5xl font-black text-black mb-12 tracking-tighter leading-none text-center">Timing & <br /><span className="bg-[#f7d302] px-4 py-1 inline-block rotate-1">Capacity</span></h2>
                        ...
                        <div className="space-y-12 mb-12">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="relative group">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full pl-16 pr-8 py-6 bg-gray-50 border-2 border-transparent focus:border-[#f7d302] focus:bg-white rounded-[32px] outline-none font-black transition-all text-xl shadow-sm appearance-none" />
                                    </div>
                                </div>
                                <div className="relative group">
                            ...
                            <div>
                                ...
                                {userVehicles.length > 0 ? (
                                    ...
                                    <div className="bg-amber-50 border-2 border-amber-100 p-12 rounded-[48px] text-center shadow-xl">
                                        <p className="text-6xl mb-6">⚠️</p>
                                        <p className="text-xl font-black text-amber-900 mb-2 uppercase tracking-tight">No Vehicle Details Found</p>
                                        <p className="text-xs text-amber-700 font-bold mb-10 leading-relaxed max-w-xs mx-auto">You need to register at least one vehicle to your profile before offering a ride to other members.</p>
                                        <button
                                        ...
                                        >
                                            Add Vehicle Now <span className="text-xl opacity-40">➔</span>
                                        </button>
                                    </div>
                                )}
                            </div>
                ...
                {step === 6 && (
                    <div className="bg-white rounded-[48px] shadow-2xl p-10 md:p-16 border border-black/5 animate-fade-in max-w-4xl mx-auto">
                        <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-16">
                            <div className="flex-1">
                                <h2 className="text-6xl font-[1000] text-black leading-[0.9] tracking-tighter mb-4">
                                    Journey <br />
                                    <span className="bg-[#f7d302] px-6 py-2 inline-block -rotate-1 skew-x-1">Economics</span>
                                </h2>
                        ...
                        <div className="flex flex-col md:flex-row gap-8">
                            ...
                            >
                                <span className="relative z-10 flex items-center justify-center gap-4">
                                    Continue to Final Audit <span className="text-3xl group-hover:translate-x-2 transition-transform">➔</span>
                                </span>
                            </button>
                        </div>
                    </div>
