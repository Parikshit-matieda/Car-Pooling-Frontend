...
const Register: React.FC = () => {
    ...
    // ← NEW: live password strength checks
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // ← NEW: catch weak password before hitting backend
        if (!isPasswordStrong) {
            setError('Password must be 8+ chars with uppercase, lowercase, number & special character (e.g. Pass@123)');
            return;
        }
            ...
            <div className="max-w-5xl w-full animate-fade-in relative z-10 grid grid-cols-1 lg:grid-cols-[0.8fr_1.2fr] gap-0 bg-white rounded-[32px] sm:rounded-[64px] overflow-hidden shadow-2xl border-4 border-black/5 h-auto lg:h-[90vh]">
                ...
                <div className="bg-black p-12 lg:p-16 flex flex-col justify-between relative overflow-hidden hidden lg:flex">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="space-y-6">
                            {[{ icon: '💸', text: 'Split Costs Instantly' }, { icon: '🌍', text: 'Zero Emission Target' }, { icon: '⚡', text: 'Lightning Fast Rides' }].map(item => (
                ...
                <div className="p-8 lg:p-14 overflow-y-auto custom-scrollbar bg-white">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="group md:col-span-2">
                                ...
                                <input
                                    ...
                                    value={formData.full_name} // ← NEW: controlled
                            ...
                            <div className="group md:col-span-2">
                                ...
                                <input
                                    ...
                                    value={formData.email} // ← NEW: controlled
                                    onChange={handleChange}
                                    className="w-full px-8 py-5 bg-gray-50 border-2 border-transparent focus:border-black focus:bg-white rounded-[24px] outline-none font-black transition-all text-xs"
                                    placeholder="ID@charusat.edu.in"
                                />
                                <p className="text-[8px] font-black text-gray-300 tracking-[0.2em] mt-3 ml-2 uppercase italic leading-loose">Must be @charusat.edu.in or @charusat.ac.in</p>
                            </div>
                            ...
                            <div className="group">
                                ...
                                <input
                                    ...
                                    value={formData.phone} // ← NEW: controlled
                                    onChange={handleChange}
                                    className="w-full px-8 py-5 bg-gray-50 border-2 border-transparent focus:border-black focus:bg-white rounded-[24px] outline-none font-black transition-all text-xs"
                            ...
                            <div className="group">
                                ...
                                <input
                                    ...
                                    value={formData.password} // ← NEW: controlled
                                    onChange={handleChange}
                                    className="w-full px-8 py-5 bg-gray-50 border-2 border-transparent focus:border-black focus:bg-white rounded-[24px] outline-none font-black transition-all text-xs"
                                    placeholder="••••••••"
                                />
                                {/* ← NEW: live password strength indicators */}
                                {formData.password.length > 0 ? (
                                    <div className="mt-3 ml-2 space-y-1">
                                        {[
                                            { ok: passwordChecks.length, text: '8+ characters' },
                                            { ok: passwordChecks.upper, text: 'Uppercase (A-Z)' },
                                            { ok: passwordChecks.lower, text: 'Lowercase (a-z)' },
                                            { ok: passwordChecks.digit, text: 'Number (0-9)' },
                                            { ok: passwordChecks.special, text: 'Special (!@#$...)' },
                                        ].map(c => (
                                            <p key={c.text} className={`text-[8px] font-black tracking-widest uppercase flex items-center gap-1 ${c.ok ? 'text-green-500' : 'text-red-400'}`}>
                                                {c.ok ? '✓' : '✗'} {c.text}
                                            </p>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-[8px] font-black text-gray-300 tracking-[0.2em] mt-3 ml-2 uppercase italic">e.g. Pass@123</p>
                                )}
                            </div>
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                                    {genderOptions.map(opt => (
                                        <button key={opt.value} type="button" onClick={() => setGender(opt.value)}
                                            className={`px-3 py-4 rounded-[20px] text-[9px] font-black uppercase tracking-widest transition-all border-2 flex flex-col items-center justify-center gap-2 ${gender === opt.value ? 'bg-black text-white border-black shadow-lg' : 'bg-gray-50 text-gray-500 border-transparent hover:border-black/20'}`}>
                        ...
                        >
                            <span className="relative z-10">
                                {loading ? 'Registering...' : <>Generate Profile <span className="inline-block group-hover/btn:translate-x-2 transition-transform">➔</span></>}
                            </span>
                        </button>
                    </form>
                    ...
                    <p className="mt-12 text-center text-[10px] font-black text-gray-400 tracking-widest">
                        ALREADY A MEMBER? <Link to="/login" className="text-black hover:bg-[#f7d302] px-2 py-1 rounded transition-colors inline-block -rotate-1 bg-black/5 ml-1 uppercase">Sign In ➔</Link>
                    </p>
                </div>
            </div>
        </div>
